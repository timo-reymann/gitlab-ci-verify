...
build-linux: create-dist ## Build binaries for linux
	...
	@go generate cmd/gitlab-ci-lint-api-proxy/main.go && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o $(BIN_PREFIX_API_PROXY)linux-amd64 $(BUILD_ARGS) cmd/gitlab-ci-lint-api-proxy/main.go
	@go generate cmd/gitlab-ci-lint-api-proxy/main.go && CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o $(BIN_PREFIX_API_PROXY)linux-arm64 $(BUILD_ARGS) cmd/gitlab-ci-lint-api-proxy/main.go
